...
parser.add_argument('--s_y', default=0.065, type=float)
...
parser.add_argument('--train_ratio', default=0.08125, type=float)
parser.add_argument('--data_ratio', default=0.125, type=float)
...
def load_mauna_loa_atmospheric_co2(ratio_data, ratio_test, xlim, linpoint=False, N_test=100):
    ...
    idx_data = np.arange(int(np.ceil(months.shape[0]*ratio_data)))
    idx_test = np.arange(int(np.ceil(months.shape[0]*ratio_test)))
    months_width = np.max(months[idx_test]) - np.min(months[idx_test])
    months = months - np.min(months)
    months = months / months_width * (xlim[1] - xlim[0])
    months = months + xlim[0]
    
    avg_ppmvs = avg_ppmvs - np.min(avg_ppmvs)
    avg_ppmvs = avg_ppmvs / months_width
    
    X = torch.from_numpy(months[idx_data]).float()
    Y = torch.from_numpy(avg_ppmvs[idx_data].reshape(-1, 1)).float()
    X_test = torch.from_numpy(months[idx_test]).float()
...
X, Y, X_test = load_mauna_loa_atmospheric_co2(args.train_ratio, args.data_ratio, [-5.0, 5.0])
N_data = X.size(0)
...
fig.tight_layout()
fig.savefig('./CO2/' + file_id + '_iid.png')
plt.close(fig)
...
fig.tight_layout()
fig.savefig('./CO2/' + file_id + '_our.png')
plt.close(fig)
...
fig.tight_layout()
fig.savefig('./CO2/' + file_id + '_gp.png')
plt.close(fig)
