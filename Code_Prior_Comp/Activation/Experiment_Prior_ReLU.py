...
Ns_Prior = [100, 1000, 3000, 5000, 10000]
Ns_Error = [100, 300, 1000, 5000, 30000]
Ns_Covar = [3000, 5000, 30000]

File_ID = 'A=RELU' + '_' + 'K=SE' + '_' + 'Q=Grid' + '_SW=' + str(Sigma_W) + '_SB=' + str(Sigma_B) + '_QN=' + str(Num_Q) + '_DN=' + str(Num_D)
...
class BNN(PyroModule):
    def __init__(self, Ldim, Qnum, S_W, S_B, obs_noise, gp_mean, gp_cov, q_method):
        ...
        def psi(z):
            return (- 1.0/2.0) * (3.0 - 6.0 * torch.pow(z,2.0) + torch.pow(z,4.0)) * torch.exp(-torch.pow(z,2.0)/2.0)
        self.phi = F.relu
        self.psi = psi
...
for i in range(len(Ns_Prior)):
    bnn = BNN([1, Ns_Prior[i], 1], Num_Q, Sigma_W, Sigma_B, 0.0, gp_mean, gp_cov, q_method)
...
#marker = mlines.Line2D([], [], color='red', marker='+', markersize=30, linewidth=3, markeredgewidth=3, label='Standard deviation')
...
for h in range(len(Ns_Covar)):
    bnn = BNN([1, Ns_Covar[h], 1], Num_Q, Sigma_W, Sigma_B, 0.0, gp_mean, gp_cov, q_method)
    bnn.set_evaluation(X_test)
...
sns.lineplot(x=X_test_np, y=GP_Cov_achr, linewidth=3.0)
sns.lineplot(x=X_test_np, y=BNN_Covs[0], ci=100, linewidth=3.0)
sns.lineplot(x=X_test_np, y=BNN_Covs[1], ci=100, linewidth=3.0)
sns.lineplot(x=X_test_np, y=BNN_Covs[2], ci=100, linewidth=3.0)
