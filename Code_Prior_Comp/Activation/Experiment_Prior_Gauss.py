...
File_ID = 'A=GAUSS' + '_' + 'K=SE' + '_' + 'Q=Grid' + '_SW=' + str(Sigma_W) + '_SB=' + str(Sigma_B) + '_QN=' + str(Num_Q) + '_DN=' + str(Num_D)
...
class BNN(PyroModule):
    def __init__(self, Ldim, Qnum, S_W, S_B, obs_noise, gp_mean, gp_cov, q_method):
        ...
        def phi(z):
            return torch.exp(-torch.pow(z,2.0)/2.0)
        def psi(z):
            return (1.0 / np.sqrt(2.0*np.pi)) * (1.0 - torch.pow(z,2.0)) * torch.exp(-torch.pow(z,2.0)/2.0)
        self.phi = phi
        self.psi = psi
