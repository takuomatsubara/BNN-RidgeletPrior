...
File_ID = 'L=4' + '_' + 'K=SE' + '_' + 'Q=Grid' + '_SW=' + str(Sigma_W) + '_SB=' + str(Sigma_B) + '_QN=' + str(Num_Q) + '_DN=' + str(Num_D)
...
class BNN(PyroModule):
    ...
    def forward(self, X, Y=None):        
        ...
        pyro.sample("obs", dist.Normal(H5, self.obs_noise), obs=Y)
        pyro.deterministic("Y", H5)
...
for i in range(len(Ns_Prior)):
    Num_H = [1, Ns_Prior[i], Ns_Prior[i], Ns_Prior[i], Ns_Prior[i], 1]
    Sig_W = [None, Sigma_W, Sigma_W, Sigma_W, Sigma_W, None]
    Sig_B = [None, Sigma_B, Sigma_B, Sigma_B, Sigma_B, None]
