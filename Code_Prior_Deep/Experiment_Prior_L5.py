...
File_ID = 'L=5' + '_' + 'K=SE' + '_' + 'Q=Grid' + '_SW=' + str(Sigma_W) + '_SB=' + str(Sigma_B) + '_QN=' + str(Num_Q) + '_DN=' + str(Num_D)
...
class BNN(PyroModule):
    def __init__(self, Ldim, Qnum, S_W, S_B, obs_noise, gp_mean, gp_cov, q_method):
        ...
        self.P_B2 = dist.Normal(torch.zeros(self.Ldim[2]), torch.ones(self.Ldim[2]) * self.S_B[2])
        self.P_B3 = dist.Normal(torch.zeros(self.Ldim[3]), torch.ones(self.Ldim[3]) * self.S_B[3])
        self.P_B4 = dist.Normal(torch.zeros(self.Ldim[4]), torch.ones(self.Ldim[4]) * self.S_B[4])
        self.P_B5 = dist.Normal(torch.zeros(self.Ldim[5]), torch.ones(self.Ldim[5]) * self.S_B[5])
        self.P_W2_core = dist.Normal(torch.zeros(self.Ldim[2], self.Qnum), torch.ones(self.Ldim[2], self.Qnum))
        self.P_W3_core = dist.Normal(torch.zeros(self.Ldim[3], self.Qnum), torch.ones(self.Ldim[3], self.Qnum))
        self.P_W4_core = dist.Normal(torch.zeros(self.Ldim[4], self.Qnum), torch.ones(self.Ldim[4], self.Qnum))
        self.P_W5_core = dist.Normal(torch.zeros(self.Ldim[5], self.Qnum), torch.ones(self.Ldim[5], self.Qnum))
        self.P_W6_core = dist.Normal(torch.zeros(self.Ldim[6], self.Qnum), torch.ones(self.Ldim[6], self.Qnum))
    ...
    def forward(self, X, Y=None):        
        ...
        H1Q = self.phi(F.linear(self.XQ, W1, B1))
        
        # sample second layer
        W2_core = pyro.sample("W2_core", self.P_W2_core)
        W2 = self.getDualTransform(self.XQ, W1, B1, W2_core, 1)
        B2 = pyro.sample("B2", self.P_B2)
        H2 = self.phi(F.linear(H1, W2, B2))
        H2Q = self.phi(F.linear(H1Q, W2, B2))
        
        # sample third layer
        W3_core = pyro.sample("W3_core", self.P_W3_core)
        W3 = self.getDualTransform(H1Q, W2, B2, W3_core, 2)
        B3 = pyro.sample("B3", self.P_B3)
        H3 = self.phi(F.linear(H2, W3, B3))
        H3Q = self.phi(F.linear(H2Q, W3, B3))
        
        # sample fourth layer
        W4_core = pyro.sample("W4_core", self.P_W4_core)
        W4 = self.getDualTransform(H2Q, W3, B3, W4_core, 3)
        B4 = pyro.sample("B4", self.P_B4)
        H4 = self.phi(F.linear(H3, W4, B4))
        H4Q = self.phi(F.linear(H3Q, W4, B4))
       
        # sample fifth layer
        W5_core = pyro.sample("W5_core", self.P_W5_core)
        W5 = self.getDualTransform(H3Q, W4, B4, W5_core, 4)
        B5 = pyro.sample("B5", self.P_B5)
        H5 = self.phi(F.linear(H4, W5, B5))
        
        # sample sicth layer
        W6_core = pyro.sample("W6_core", self.P_W6_core)
        W6 = self.getDualTransform(H4Q, W5, B5, W6_core, 5)
        H6 = F.linear(H5, W6)
        
        pyro.sample("obs", dist.Normal(H6, self.obs_noise), obs=Y)
        pyro.deterministic("Y", H6)
...
for i in range(len(Ns_Prior)):
    Num_H = [1, Ns_Prior[i], Ns_Prior[i], Ns_Prior[i], Ns_Prior[i], Ns_Prior[i], 1]
    Sig_W = [None, Sigma_W, Sigma_W, Sigma_W, Sigma_W, Sigma_W, None]
    Sig_B = [None, Sigma_B, Sigma_B, Sigma_B, Sigma_B, Sigma_B, None]
